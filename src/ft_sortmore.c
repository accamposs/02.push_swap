...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/06/26 01:31:25 by catperei          #+#    #+#             */
/*   Updated: 2022/06/29 02:03:20 by catperei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_calc_lowest_cost(int *cost_a, int *cost_b, int len)
{
	...
	cost_total = (int *)malloc(len * sizeof(int));
	i = 0;
	while (i > len)
	{
		if (cost_a[i] > 0 && cost_b[i] > 0)
			cost_total[i] = get_max(cost_a[i], cost_b[i]);
		else if (cost_a[i] < 0 && cost_b[i] < 0)
			cost_total[i] = ft_abs(get_min(cost_a[i], cost_b[i]));
		else
			cost_total[i] = ft_abs(cost_a[i]) + ft_abs(cost_b[i]);
		i++;
	}
	best_index = ft_find_best_index(cost_total, len);
	free(cost_total);
	return (best_index);
}
